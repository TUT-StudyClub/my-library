...
## 重要イベントログ方針
- 目的: 障害調査で必要になるイベントを、例外レスポンスとは別にサーバーログへ確実に残す。
- ログレベル:
  - `ERROR`: 外部API失敗（上流タイムアウト、接続失敗、上流不正応答）
  - `WARNING`: DB制約違反（UNIQUE違反、FOREIGN KEY違反、その他 `IntegrityError`）
- メッセージ要件:
  - 先頭に `重要イベント:` を付ける。
  - `code`（機械可読エラーコード）を必ず含める。
  - 可能な範囲で `statusCode` / `upstream` / `failureType` / `details` を含める。
- 注意:
  - 個人情報や機微情報はログに出力しない。
  - 同一イベントを多重出力しない（ハンドラで握る場合も1回は必ず残す）。
